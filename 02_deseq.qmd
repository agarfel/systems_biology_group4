...
```{r}
geneCountMatrix_Ocular = read.csv("data/OcularGeneCountMatrix.csv", row.names = 1)
geneCountMatrix_Spinal = read.csv("data/SpinalGeneCountMatrix.csv", row.names = 1)
...
```{r}
fg_sig_ocular |> filter(padj < 0.05)|> pull(pathway)
fg_sig_spinal |> filter(padj < 0.05)|> pull(pathway)
```
...
fg_nondir_ocular |> filter(padj < 0.05)|> pull(pathway)
fg_nondir_spinal |> filter(padj < 0.05)|> pull(pathway)
```
...
p1 <- ggplot(fg_volcano_ocular, aes(x = NES, y = mlog10padj, color = sig)) +
...
ggsave("volcano_ocular.png", p1, width = 6, height = 6, dpi = 300)

p2 <- ggplot(fg_volcano_spinal, aes(x = NES, y = mlog10padj, color = sig)) +
  geom_point(alpha = 0.8) +
  geom_hline(yintercept = -log10(0.05), linetype = 2) +
  ...
  theme_minimal(base_size = 11)
ggsave("volcano_spinal.png", p2, width = 6, height = 6, dpi = 300)
```
